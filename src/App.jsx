...
const PRIVACY_SECTIONS = [
    ...
    { id:"privacy-3", title:"第3条 個人情報の取り扱い一覧・データ台帳", body:[
      ...
      rows:[
        ...
        ["応募・承認・作業の記録（応募日時・承認日時・開始と終了の確認・出欠・欠勤への異議）","それぞれの操作の時","応募状況の管理。実績の表示。紛争への対応。不正利用の防止。","応募テーブル","当事者の双方に表示されます。集計した実績（完了件数など）は将来、プロフィールに表示されます","アカウントの存続中。退会後は、個人を特定できない形にして保存します。"],
...
export default function App(){
  ...
  useEffect(() => {
    ...
    const _subTabOfAdmin = (tab === "admin") && (_curHash.startsWith("admin/review/") || _curHash === "admin/consignment" || _curHash.startsWith("admin/consignment/") || _curHash === "admin/working" || _curHash.startsWith("admin/working/"));
  ...
  return(
    <div style={{minHeight:"100vh",background:C.washi,color:C.ink,"--mode-accent":modeAccent,"--role-accent":(me && !empCtx) ? ROLE_ORANGE : ROLE_GREEN,"--role-accent-soft":(me && !empCtx) ? "rgba(247,107,28,0.15)" : "rgba(0,168,107,0.13)"}}>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"16px 24px 72px"}}>
        ...
        {!needsAccountHolder&&!openAccountForm&&!chatAppId&&!applyPage&&safeTab==="admin"&&isAdmin(me)&&!consignRoom&&!workingRoom&&<Suspense fallback={<p className="f-sans" style={{ textAlign:"center", color:"#999", fontSize:13, padding:"40px 0" }}>読み込み中<Dots /></p>}><AdminTab
