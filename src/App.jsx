...
export default function App(){
  ...
  const [timelessRoom,setTimelessRoom]=useState(()=>{ try { return window.location.hash.replace(/^#\/?/,"").startsWith("admin/timeless"); } catch { return false; } }); // 農タイムレス（#/admin/timeless・管理者専用・2026-08-31に委託レーンから独立）
  ...
  useEffect(() => {
    const onHash = () => {
      ...
      setTimelessRoom(rawHash.startsWith("admin/timeless"));
    ...
    <div style={{minHeight:"100vh",background:C.washi,color:C.ink,"--mode-accent":modeAccent,"--role-accent":(me && !empCtx) ? ROLE_ORANGE : ROLE_GREEN,"--role-accent-soft":(me && !empCtx) ? "rgba(247,107,28,0.15)" : "rgba(0,168,107,0.13)"}}>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"16px 24px 72px",minHeight:"100svh",boxSizing:"border-box"}}>
        ...
        {/* エラーの運営チャットへの報告は、DBのトリガー（trg_z_app_error_report・migration 20260831060413）が
            app_errors への記録の瞬間に管理者のスレッドへ入れる＝フロントの投函（AdminErrorChatReporter）は撤去。
            誰の端末のエラーでも、管理者の端末が閉じていても届く。宛先は管理者本人のスレッド1本だけ＝利用者には見えない */}
        ...
        {!needsAccountHolder&&!openAccountForm&&!needsPrivacyReconsent&&!chatAppId&&!applyPage&&safeTab==="admin"&&isAdmin(me)&&timelessRoom&&<Suspense fallback={<p className="f-sans" style={{ textAlign:"center", color:"#999", fontSize:13, padding:"40px 0" }}>読み込み中<Dots /></p>}><FarmTimelessRoom/></Suspense>}
