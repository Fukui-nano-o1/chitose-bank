...
export function ProfileHub({ me, onNewJob, onResume, savedDraftJobNumber, onDismissDraftSaved, onAvatarChange, onLogout }) {
  const hashToPTab = () => {
    ...
    <div className="profile-employer-edge" style={{maxWidth:1024,margin:"0 auto",padding:"32px 4px 0"}}>{/* プロフィール両面とも画面端から10pxに統一（モバイル・CSS側の負マージン併用） */}
      ...
      {pTab === "worker" ? (
          ...
          <>
            ...
            {(() => {
              return (<>
                ...
                {/* 「新しく求職を出す」カードは削除（2026-08-22たきと指示）。
        ...
        <>
          ...
          <FarmerDashboard onNewJob={onNewJob} onResume={onResume} savedDraftJobNumber={savedDraftJobNumber} onDismissDraftSaved={onDismissDraftSaved} me={me} />
        </>
      )}
      </div>
