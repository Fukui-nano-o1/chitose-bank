...
export default function LaborConditionsNotice({ me, role = "worker" }) {
  ...
  return (
    <>
      {/* 入口カード：件数0でも出す（タップ不能・非表示にしない＝2026-08-03の原則。中で説明を出す） */}
      <button onClick={() => setListOpen(true)} className="f-sans" style={{ width:"100%", marginTop:12, background:"#fff", border:"1px solid #EBEBEB", borderRadius:20, padding:"18px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:14, textAlign:"left", boxShadow:"0 2px 12px rgba(0,0,0,0.05)" }}>
        ...
        <span style={{ flex:1, minWidth:0 }}>
          ...
          <span className="f-sans" style={{ display:"block", fontSize:13, color:"#717171", marginTop:4, lineHeight:1.6 }}>
            {rows === null ? "読み込み中…" : count > 0 ? `採用が決まった時の労働条件です。${count}件　表示・印刷できます` : "採用が決まると、その時の労働条件がここに残ります"}
          </span>
        </span>
      </button>
