...
export function InsurancePanel({ employer }) {
  ...
  // 1項目=1ボックス（アイコンなし・タップで説明展開）。muted=「これから準備予定」用の淡色トーン
  const renderBox = (k, muted) => {
    ...
    return (
      <div key={k} style={{ background: muted ? "#FAFAFA" : "#fff", border:"1px solid #EBEBEB", borderRadius:12, overflow:"hidden" }}>
        <button
          onClick={()=>setOpen(p => ({ ...p, [k]: !p[k] }))}
          aria-expanded={isOpen}
          className="f-sans"
          style={{ width:"100%", background:"none", border:"none", padding:"14px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, cursor:"pointer", textAlign:"left" }}
        >
          <span style={{ fontSize:14, fontWeight:700, color: muted ? "#717171" : "#222", minWidth:0 }}>{it.chip}</span>
  ...
  return (
    <div className="f-sans">
      ...
      {/* 加入済みの保険＝ボックス並び。「これから準備予定(considering)」は性質が違う（未加入）ので下に分離して淡色ボックス */}
      <div style={{ display:"grid", gap:10 }}>
        {items.filter(k => k !== "considering").map(k => renderBox(k, false))}
      </div>
      {items.includes("considering") && (
        <div style={{ marginTop: items.some(k => k !== "considering") ? 14 : 0 }}>
          {renderBox("considering", true)}
        </div>
      )}
    </div>
  );
