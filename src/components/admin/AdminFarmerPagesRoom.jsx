...
const STEPS = [
  ...
  { ch:"準備", name:"農家をはじめる", url:"#/profile/worker → #/profile/employer", act:"プロフィール入口の浮遊ボタン「農家に切替」で、雇い手の顔に切り替える。登録の時点では役割を聞かれない。",
    body: () => (
      <div style={{ padding:14, position:"relative", minHeight:260 }}>
        ...
        <div style={{ position:"absolute", right:14, bottom:14 }}>
          <span className="f-sans" style={{ display:"inline-block", padding:"12px 24px", borderRadius:24, background:ROLE_GREEN, color:"#fff", fontSize:17, fontWeight:700, boxShadow:"0 4px 12px rgba(0,0,0,.18)" }}><NavIconInline name="swap" size={17} />農家に切替</span>
        </div>
      </div>
    ) },
  ...
  /* ═══ 仕事の当日 ═══ */
  { ch:"仕事の当日", name:"つぎの予定（マイページ）", url:"#/profile/employer", act:"7日以内の予定が近い順に並ぶ。行をタップでその相手との予定詳細へ。日程・状態を確認し、チャットや労働条件通知書を開ける。",
    body: () => (
      <div style={{ padding:14 }}>
