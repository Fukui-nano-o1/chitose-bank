                  ...
                  {INSURANCE_ITEMS.map(it => {
                    ...
                    return (
                      <button key={it.k} onClick={()=>setInsOpenKey(open ? null : it.k)}
                        className="f-sans" style={{ flexShrink:0, maxWidth:200, background: open ? "#EAF7F1" : "#F7F7F7", border:"1px solid " + (open ? ROLE_GREEN : on ? "#BFE6D5" : "#EBEBEB"), borderRadius:12, padding:"10px 14px", cursor:"pointer", textAlign:"left" }}>
                        <span style={{ display:"block", fontSize:13, fontWeight:700, color: open ? ROLE_GREEN : "#222", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{it.icon} {it.chip}</span>
                        <span style={{ display:"block", fontSize:11, color: on ? ROLE_GREEN : "#B0B0B0", marginTop:2 }}>{on ? "申告ずみ ✓" : "未申告"}</span>
                      </button>
                    );
                  })}
                </div>
