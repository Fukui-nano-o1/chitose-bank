...
const PAGE_GUIDES = [
  ...
  },
  {
    ...
    match: (h) => h === "login",
    lead: "登録済みなら再登録は不要です。登録したメールアドレスとパスワードでログインできます。",
    rows: [
      { icon: "mail", t: "登録済みの方は「ログイン」", d: "新規登録やコード入力の画面からも、「パスワードでログイン」に切り替えられます" },
      { icon: "sprout", t: "はじめての方は「新規登録」", d: "6桁コードで確認します。登録済みなら「ログインして続ける」、はじめてならパスワード設定へ進みます" },
      { icon: "lock", t: "パスワードを忘れたら", d: "「パスワードを忘れた方・未設定の方」から、確認コードで再設定できます" },
    ],
    spots: [
  ...
  // モバイルの☰からは開き直せない＝自動表示の一度きりが主な入口
  {
    ...
    match: (h) => h.startsWith("chat/") && !h.startsWith("chat/admin"),
    ...
    rows: [
      ...
      { icon: "flag", t: "「詳細」から仕事や通報を確認", d: "仕事の内容・日程・集合場所や、問題のあるメッセージの通報が開けます。画面の不具合は「この画面を報告」から送れます" },
    ],
    spots: [
      { sel: '[data-guide="chat-input"]', label: "ここに書いて「送信」。相手にすぐ届きます。" },
      { sel: '[data-guide="chat-partner"]', label: "相手の名前です。タップするとプロフィールが見られます。" },
      { sel: '[data-guide="chat-details"]', label: "仕事の詳細と、メッセージの通報はこちらから。" },
    ],
  },
  ...
  },
  {
    ...
    rows: [
      { icon: "star", t: "選択式で振り返ります", d: "仕事の評価は基本3問＋任意3問です。自由記述はありません" },
  ...
  },
  {
    ...
    lead: "相手とのやり取りの一覧です。",
    rows: [
      { icon: "chats", t: "仕事ごとに1つの会話", d: "相手の名前・仕事・求人番号で検索し、未読・進行中・終了で絞り込めます" },
  ...
  {
    key: "scheduleDetail", title: "予定の詳細", art: "calendar",
    match: (h) => /^profile\/(employer|worker)\/schedule\//.test(h),
    lead: "相手ごとの日程と進行状況を確認する画面です。",
    rows: [
      { icon: "calendar", t: "日程を確認", d: "合意した日や希望日と、勤務時間が見られます。採用前の予定はその旨を表示します" },
      { icon: "chats", t: "相手に連絡", d: "メッセージのボタンから、この相手とのチャットを開けます" },
      { icon: "postJob", t: "条件を確認", d: "採用後は、この仕事の労働条件通知書を開けます" },
    ],
    spots: [],
  },
  {
    ...
    rows: [
      ...
      { icon: "clock", t: "つぎの予定", d: "7日以内の予定が並びます。タップすると相手との予定詳細が開きます" },
    ],
