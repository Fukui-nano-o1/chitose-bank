      ...
      <div style={{ display:"grid", gap:12 }}>
        {[
          ...
          <div key={item.label} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:"#fff", borderRadius:10 }}>
            <span style={{ display:"flex", color:"#717171" }}>{item.icon.length <= 2 ? <span style={{ fontSize:18, lineHeight:1 }}>{item.icon}</span> : <NavIcon name={item.icon} size={18} />}</span>
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#fff", zIndex:9999 }}>
      ...
      {isEditing && (
        ...
        ><NavIcon name="close" size={15} /></button>
      )}
