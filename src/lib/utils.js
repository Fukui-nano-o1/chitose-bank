...
// 規約v2.5 = 2026-08-19 改訂（第4条に緊急連絡先の4項＝初期値の自動登録・本人確認までは相手方に
...
// 規約v2.8／プラポリv4.4 = 2026-08-24 同日の再改訂（信頼カードから「求人内容との一致」の集計を
// 削除したので、第8条2四を「肯定的な回答の件数だけを集計として表示する」に戻し、二も「他の利用者に
// 表示されません」の平明な書き方へ。プラポリの例示も「また働きたい 3件」に差し替えた。
// ★否定の件数を出す表示を復活させるなら、四と二を同時に見直すこと（表示と文面は必ず対で動かす）
// 規約v2.7 = 2026-08-24 改訂（第8条2四から「一定件数以上」を削除＝集計は件数にかかわらず表示する。
...
// 規約v2.9 = 2026-08-25 改訂（第8条2一＝自由記述は運営の承認を経て公開、を「送信の時点で表示・
...
export const PRIVACY_VERSION = "v4.7-2026-09";
...
//   ②今日ページの「👤プロフィール入力」ボックスのバッジ（箱は常設・2026-08-19。数が0ならバッジ無し）
