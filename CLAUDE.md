...
━━━ 2026-08-22 24h判定＝【負け】・昨日の帰属は誤りだった（訂正）＋真犯人はpgssに写らない層 ━━━
【判定】work_mem=16MB 適用後の24h（06:28→06:38 UTC）＝ temp +2,650ファイル・+10.4GB。
ゼロ近傍どころか歴史平均（1.9GB/日）の5倍。効果判定は【負け】。
【★訂正（昨日の帰属は二重に誤り）】
1. 内省クエリ（with f as -- CTE with sane arg_modes…）は同じ24hで【+1回・+9MB】しか動いていない。
   entry の stats_since=04-11 が生存＝「pgssから追い出された」という昨日の推定も反証された。
   実際の実行は4.5ヶ月で207回（1.5回/日）＝「再構築75回/日×9.1MB」の積算は成立しない
   （Schema cache queried のログ行と、この内省クエリの実行は1:1でない）。
2. so authenticator への work_mem=16MB は【無罪の役への薬】だった。無害so残置するが、
   32MBへの引き上げは意味がない＝【やらない】（送信予約していた提案は撤回）。
【確定した事実（本セッション実測）】
・pgssに写る temp の生涯合計=1.95GB vs DB全体285GB＝【99.3%がpgssの外】で書かれている
・クラスタ最大のリレーション=2.4MB（全スキーマ）＝正当なデータソートでGB級は物理的に不可能
・supabase_admin が常駐4接続。`set pg_stat_statements.track = none` が1,800回
  （=接続確立ごと・135日で13回/日）＝プラットフォームの観測セッションは pgss に写らない設計
・pgss全走査は1回4.7〜9.4MBをspill（自分の診断クエリで実測・work_mem 2184kB）。
  10.4GB/日÷4.7MB≈2,200回/日の算術が成立＝プラットフォーム観測がpgss等を走査している仮説が最有力
・書き込みはバースト型（42秒の実測窓で+0）。常駐ポーラーpid 2461514の正体はレプリカ健全性
  チェック（3分周期・spillしない）＝これも犯人ではない
・log_temp_files は 42501 permission denied＝中からの確定は不可能（superuser専用）
・参考：cron 2本（auto-start-work・job-time-notices）が08-21 12:05に job startup timeout で
  同時失敗＝その分にリソース逼迫があった可能性（バーストの時刻の手がかり・未確定）
【教訓（測定の作法に追加）】帰属は「per-call固定値×ログの回数」の積算だけで確定しない。
必ず【対象そのものの calls / stats_since の差分】で裏を取る（今回それで一日で誤りが割れた）。
pgss は track=none のセッションと完了しなかった文を写さない＝「pgssに無い」は「存在しない」ではない。
【起票文】docs/supabase-support-ticket-2026-08-21.md を全面改訂（2026-08-22版が正・初版のまま
提出しないこと）。新しい中身＝10.4GB/日がpgss不可視／log_temp_files有効化の依頼／
プラットフォーム観測へのwork_mem手当の依頼／Realtime再構築の嵐はCPU・プール問題として維持
（ディスクの帰属は撤回と明記）。★提出＝たきとのPC作業（ダッシュボード→Support）
【残タスク】
1. たきと：起票文の提出（改訂版）。あわせて supabase CLI の postgres-config で
   log_temp_files を当てられるか調査（PC作業・当てられれば自前で犯人の名前が出る）
2. D+A+B「リクエスト削減」PR（半日）＝Disk IOとは独立の11月対策として承認済み・着手指示待ちのまま
3. 32MB引き上げ＝やらない（本ブロックで撤回）。16MBは残置
━━━ ここまで ━━━
