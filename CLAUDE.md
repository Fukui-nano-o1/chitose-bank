...
・lib/utils に entryWorkDays(entry) を新設＝【カレンダーに出す日の唯一のソース】。
  優先順：agreed_dates（確定）＞ available_dates（申請された希望・未確定）＞ 求人期間。いずれも holidays を除く。
  available_dates は配列のほか "any"（期間中いつでも）と null（単日求人）を取り、どちらも期間へ倒す
  （＝日を絞っていない）。返り値 kind（agreed/available/range）が画面の描き分けの目印。
...
・★希望日はまだ確定ではないので見た目で分ける：その日に出ている予定が全部 available 由来なら斜線
  （repeating-linear-gradient）にし、ベタ塗り（＝決まった日）にしない。凡例に
  「来られる日（働く日は未確定）」を1行追加。農家が働く日を決めると、その日はふつうの塗りに変わる。
【検証】build成功・eslint 0 error（警告29＝stash比較で変更前と同数＝新規ゼロ）・distに新文言を包含確認。
entryWorkDays を実ソースからimportしてnodeで17ケース機械検算（優先順・"any"・単日・休日を3経路から除く・
空配列は次の段へ倒す・壊れた入力で無限ループしない=400打切・従来動作の回帰）＝全OK。
DBはロールバック付きで実測：新2列が届くこと、配列＋休日の経路、権限（anon不可/authenticated可）。
残置ゼロ実測（合成値は巻き戻し済み。available_dates が1件残るのは 2026-07-30 作成の実データ）。
【実機目視の残り】①承認済みの期間求人で、カレンダーが希望日だけを斜線で塗るか（期間全部が埋まらないか）
②農家が「働く日を決める」を押すと、その日がふつうの塗りに変わるか ③名前チップの出る日が塗りと一致するか
