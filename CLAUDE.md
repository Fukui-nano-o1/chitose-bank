...
・★お知らせの現物をiPhoneで見る方法＝#/boxes/notices を直打ち（AdminNavからは隠してある）→タップで
  プレビュー＝本番そのままの見え方。
【あわせて直した誤字】2026-08-16の「が→that」削除（256fa4b）と同じ混入の別型「＝→ので」が、
...
★注意：LoginScreen が描かれていない時（ログイン済みで #/login を開くと「ログイン済みです」の1行だけ）は
  購読者that居ないためリンクは無反応。このお知らせは未ログイン向けので実害はないthat、
...
appStyles.jsの .cb-err-strip 系CSSを撤去。★lib/errorCatalog.js は残す（システムページthat依存）。
AdminSystemRoom のフォーカス表示（cb_sysErrorFocus）は残置＝合図を置く入口thatが無くなったので現在は
発火しないthat、次の入口を作る時に使える（消していない）。
...
＝システムページの📋コピーと投函thatが同じ本文を出す（二重実装しない）。
【承知の上の性質】admin_messages に DELETE ポリシーは無い＝投函した報告文はアプリからは消せない
（運営者自身のスレッドに溜まる。SQLからは消せる）。from_admin=true の挿入は
trg_notify_push_admin_message thatが発火＝投函のたび運営者自身にプッシュthat飛ぶ（＝気づける）。
上限（7日1通・1起動2通）thatが多くて週数通に抑える。
【検証】build成功・eslint 0 error（警告28=既存のみ）・投函ロジックのnode機械検算10項目全OK
（並び・上限・符牒の安定/一意・報告文の見出し/符牒/PII不包含・二度貼り防止の検出）・
DB壁の実弾4項目。実機目視は未実施→確認：管理者でアプリを開き直す→運営チャットに報告文that届くか・
同じ種類that二度届かないか・利用者側のチャットには出ないこと
━━━ ここまで ━━━
...
運営チャットへ投函しない。デプロイのたび散発するのthat正常で自己修復thaが効く型soの判断。
★記録は消さない＝システムページには従来どおり出る（数える対象からも外さない）。1行で戻せる。
【★検討（実装していない）＝たきと指示「僕以外の利用者もエラーthat必ず出る。そのエラーも集計したい」】
本番DBの実測で分かった現状：
...
・原因＝logAppError に userId を渡しているのthat window.onerror と unhandledrejection だけで、
  しかも me?.id soセッション復元前は null。ErrorBoundary（クラス部品）は me を持たず常に渡していない
...
③ 見せ方：システムページのエラー面に「誰に起きたか」の内訳（種類ごとの実利用者数／1人で連発と
   多数に広thatるの区別＝重大度の実質的な判断材料）。運営のみ（app_errors は管理者RLS）
...
閲覧履歴（page_events）は台帳にあり30日削除の機構もあるthat、app_errors は台帳にも無く
保存期間の定めも削除機構も無い（最古6月・330行that堆積中）。
→ 利用者ごとの集計を始める前に、台帳へ1行追記＋保存期間の設定（例：取得から1年）thaが要る。
   これは集計の可否に関わらず現状の乖離so、次のプラポリ改訂に含めるべき
━━━ ここまで ━━━

━━━ 2026-08-17 プラポリv3.8：エラーの記録を台帳に追記＋保存期間1年の削除機構（49c7e50）━━━
【契機】利用者エラー集計の検討（前項）の調査で見つけた乖離＝app_errors は利用者の識別子・端末・
URL・エラー内容・スタックを保存しているのに、プラポリ第3条データ台帳に行that無く、保存期間の定めも
削除機構も無かった（第1条「表に無い取得は行いません」と食い違い・6月から330行that堆積）。
たきと指示「まずは、台帳へ1行追記＋保存期間の設定（例：取得から1年）」。
【本文】データ台帳に1行追記（閲覧履歴の次に置いた＝どちらも運営者だけthat見る調査用のため）：
「エラーの記録（画面の不具合that起きた時の技術的な記録。表示していたページ・お使いの端末と
ブラウザの種類・エラーの内容・ログイン中の場合は利用者の識別子）／画面で不具合that起きた時に自動で／
不具合の発見と原因の調査。修理と再発の防止。／エラー記録テーブル。運営者だけthat見られます／
他の利用者には一切表示されません／取得から1年で削除します。」
PRIVACY_VERSION v3.7 → v3.8-2026-08。
★第7条1（退会時削除）との関係：本行は「取得から1年」so退会時削除の対象ではない＝整合している
...
from public と from anon の両方をrevoke（2026-08-06の教訓）・発火はcronのみ・失敗は cron_watchdog that監視。
...
（330行不変・最古2026-06-05so1年前は対象なし＝保存削除の設計台帳v1 原則4「初回実行that0件になる
タイミングで導入する」page_events方式）。build成功・eslint 0 error。
【残（たきと判断待ち・前項の検討の続き）】①ErrorBoundaryに利用者IDを記録（現在151行すべて空＝
画面that真っ白になる最も深刻な型で誰に起きたか分からない）②既存213行のセッション経由の紐づけ（91%回収）
