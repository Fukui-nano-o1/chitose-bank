-- 今日のやることに「求人への質問」を追加（2026-07-27たきと指示）
-- 求人Q&A（第10弾・job_questions）の未回答＝農家のアクション待ち。求人カードのバッジ❓Nと同じ母集団を、
-- 今日ページのボックスにも並べる。1行=1質問（質問者の名前・アイコンを partner_* に載せる）。
-- 行き先は求人詳細の「質問」タブ（フロント側で cb_jobTab を消費）。
-- 既存CTEには一切触れず fq を追加して union するだけ（追加可能・他を壊さない）。
