...
--
-- 【何that起きていたか】2026-08-03の anon マスク徹底（20260803131655）で、jobs_public は
--   ・座標 lat/lng を小数2桁（約1.1km格子）へ丸める
--   ・geo_radius_m を3000m以上に広げる
--   ・nearest_station（最寄り駅名）を伏せる
-- を入れていたthat、2026-08-06の 20260806013953（終了した求人も並べる）で、
-- マスク導入前のビュー定義を土台に create or replace したため、この3点thatまるごと外れていた。
-- 町域・募集主・番地のマスクは残っていたので気づきにくい状態だった。
--
-- 【実測（本番・anonロールで実行）】座標の小数桁=6・半径の最小=500・駅名thatある行=12/12
-- ＝町域を伏せていても、地図に載せれば就業場所thatほぼ特定できる状態だった。
...
-- 過去のマイグレーションからコピーすると、その後に入ったマスクthat静かに消える。
...
create or replace view public.jobs_public as
 select j.job_number,
    ...
    j.photos,
    j.created_at,
    -- 座標は訪問者には約1.1km格子へ丸める（町域that特定できない粒度・2026-08-03）
    ...
    -- 丸めた座標に見合う広さにする（座標that無い求人は従来どおりNULL）
