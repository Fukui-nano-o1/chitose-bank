-- 採用押し忘れを運営that拾える導線（2026-08-06 実機一周②・#1054/#1056の教訓(1)への対処）
--
-- 【何that問題か】採用（雇い手の確認＝confirm_terms）を押し忘れたまま作業開始時刻を迎えると、
-- cron（expire_stale_applications）that応募を失効させ、実際に働いた事実thatあっても記録thatない状態になる
-- （2026-08-05後追い記録 #1054・#1056）。作業当日を過ぎた採用済み未処理を運営that拾う導線は
-- #/admin/working にあるthat、採用そのものの押し忘れは誰にも見えないまま失効していた。
--
-- 【対処＝2点・どちらも既存の記録原則のまま（状態の上書きなし・導出表示のみ）】
-- 1. 承認まで進んでいた応募（approved/meeting/interview）that自動終了（失効・満員見送り）した時、
--    運営へメールで知らせる（send_admin_email＝鍵が無ければ静かに戻る既存関数）。
--    「承認済みのまま終了」＝働いた実態thatあった可能性の瞬間なので、運営thatその場で実態を確認できる。
-- 2. 前日督促（9時台）を approved・採用未確定にも拡大（従来は applied のみ）。
--    #1054の型（承認したまま採用を確定し忘れる）を、失効する前日に本人へ知らせて防ぐ。
--
-- 【変えていないもの】失効・見送りの判定条件／働き手への通知とメール／状態遷移／cronスケジュール。
