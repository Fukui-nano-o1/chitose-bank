-- 採用押し忘れを運営が拾える導線（2026-08-06 実機一周②・#1054/#1056の教訓(1)への対処）
--
-- 【何が問題か】採用（雇い手の確認＝confirm_terms）を押し忘れたまま作業開始時刻を迎えると、
-- cron（expire_stale_applications）が応募を失効させ、実際に働いた事実があっても記録がない状態になる
-- （2026-08-05後追い記録 #1054・#1056）。作業当日を過ぎた採用済み未処理を運営が拾う導線は
-- #/admin/working にあるが、採用そのものの押し忘れは誰にも見えないまま失効していた。
--
-- 【対処＝2点・どちらも既存の記録原則のまま（状態の上書きなし・導出表示のみ）】
-- 1. 承認まで進んでいた応募（approved/meeting/interview）が自動終了（失効・満員見送り）した時、
--    運営へメールで知らせる（send_admin_email＝鍵が無ければ静かに戻る既存関数）。
--    「承認済みのまま終了」＝働いた実態があった可能性の瞬間なので、運営がその場で実態を確認できる。
