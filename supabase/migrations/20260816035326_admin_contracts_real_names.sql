...
-- 契約の記録＝雇用の法定手続きの文脈so、当事者はニックネームでなく本名で示す（2026-07-30裁定(B)と整合）。
...
--        ③ニックネーム（本名that無い場合の最後の砦・表示thatー にならないため）
-- 管理者専用RPC（app_adminsゲート）＝運営は本人確認台帳を扱う立場so本名表示は開示範囲の拡大ではない。
-- 実測：#1053（凍結前の旧契約）の働き手名that本名と一致・ニックネームと不一致。フロント変更なし（同じキー名）。
