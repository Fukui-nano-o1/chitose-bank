-- ユーザー入力をHTMLメールに埋める際のエスケープ（開放前の必須修理）
create or replace function public.h(p text)
returns text language sql immutable as $$
  select replace(replace(replace(replace(replace(coalesce(p,''),
    '&','&amp;'), '<','&lt;'), '>','&gt;'), '"','&quot;'), '''','&#39;');
$$;

-- 最大露出点：応募メール（他人の自由記述が農家メールに入る）を修理
create or replace function public.trg_notify_application()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  v_nickname text; v_pr text; v_avatar text; v_crop text; v_task text;
...
begin
  ...
  v_html :=
    ...
    || '　' || public.h(v_crop) || ' ' || public.h(v_task)
