-- 働き手の「求人内容の確認」を廃止（2026-07-25たきと指示：内容を確認した上で応募するのが前提）。
-- ①応募INSERT時に terms_confirmed_worker_at を自動記録（応募＝内容確認済み。採用成立の判定式
--   〈hired_count等＝両者確認〉を一切変えずに済む方式）
-- ②既存行をバックフィル（契約凍結済み〈terms_snapshot有り〉の行は凍結トリガー保護のため対象外）
-- ③my_todo_items v9：w_confirm 段を削除。日程の申請（チャットの候補日・来られる日）は無変更で残す
create or replace function public.stamp_worker_terms_on_apply()
returns trigger
language plpgsql
security definer
...
as $trg$
begin
  new.terms_confirmed_worker_at := coalesce(new.terms_confirmed_worker_at, now());
  return new;
end; $trg$;
drop trigger if exists trg_stamp_worker_terms_on_apply on public.applications;
create trigger trg_stamp_worker_terms_on_apply
  before insert on public.applications
  for each row execute function public.stamp_worker_terms_on_apply();

update public.applications
   set terms_confirmed_worker_at = created_at
 where terms_confirmed_worker_at is null
   and terms_snapshot is null;

-- my_todo_items v9（v8からの差分はw_confirm削除のみ）：
...
--   ・働き手の求人内容の確認（w_confirm）は廃止（応募＝内容確認済みの前提）
