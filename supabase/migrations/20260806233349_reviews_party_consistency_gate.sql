...
-- 穴：reviews の INSERT ポリシーは「投稿者that application_id の当事者か」しか見ておらず、
-- reviewee_id と direction that その応募の相手方・向きと一致するかを検証していなかった。
-- ＝当事者である自分の応募に、被評価者を【無関係の第三者】にすり替えた評価を捏造でき、
--   worker_trust_info に反映されて任意の相手の評判を汚染できた（実測 0→1）。
-- 対処：BEFORE INSERT トリガーで、レビュー行thatその応募の当事者関係・向きと厳密に一致することを
...
-- ★status='completed' 要求は入れない：働き手経路は confirm_end 直後（completed遷移前の瞬間thatあり得る）so、
--   状態要求で正当な評価を弾く危険thatある。捏造は「reviewee のすり替え」that本体so、
...
begin
  ...
  if v_worker is null then
    raise exception 'レビューの応募that存在しません';
  end if;

  if new.direction = 'farmer_to_worker' then
    if new.reviewer_id <> v_farmer or new.reviewee_id <> v_worker then
      raise exception 'レビューの当事者that応募と一致しません（farmer_to_worker は 農家→働き手のみ）';
    end if;
  elsif new.direction = 'worker_to_farmer' then
    if new.reviewer_id <> v_worker or new.reviewee_id <> v_farmer then
      raise exception 'レビューの当事者that応募と一致しません（worker_to_farmer は 働き手→農家のみ）';
    end if;
  else
    raise exception 'direction that不正です: %', new.direction;
  end if;
