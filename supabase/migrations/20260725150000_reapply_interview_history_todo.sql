-- 二頭運転の衝突修復（2026-07-25）：apply_is_consent_drop_wconfirm が旧定義ベースで
-- my_todo_items を上書きし、「面接の質問＝送信履歴（interview_question_sends）基準」の判定が
-- 本文パターン照合に戻っていた。現定義（w_confirm削除済み）を維持したまま finterview のみ再適用。
-- （DBには2026-07-25にMCP直接適用済み。本ファイルは正本の写経）
...
as $function$
  ...
  finterview as ( -- 面接の質問を送る：送信履歴（interview_question_sends）が無い応募だけ計上（2026-07-25再適用）
