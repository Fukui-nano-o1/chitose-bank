...
--   位置系3点（座標の小数2桁丸め・半径3000m・最寄り駅の非表示）だけthat静かに外れていた。
--   町域・募集主3項目・番地のマスクは残っていたため気づきにくい（2026-08-06 実害1と同型・2回目）。
--   実測：anon that 座標6桁・半径500・駅名9/9件を取得できた＝地図に載せれば町域を伏せた意味が消える。
--
...
--   点検 audit.sql ① thatこの型を検出する（今回も検出した）＝ビューを触った日は必ず流すこと。
