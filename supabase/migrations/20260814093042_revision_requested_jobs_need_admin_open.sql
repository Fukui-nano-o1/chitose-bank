...
--   公開中の求人を draft に戻し修正を依頼する事後モデレーション）を、農家が無修正のまま
...
--      運営にアプリ内お知らせ＋メール（審査プレビューへのリンク）を送り、運営が確認して開く
...
--      pending の間フラグが残る＝運営がプレビューで「修正のお願い中の再提出」だと分かる。
--      運営が開いた瞬間（pending→open）にフラグが消える＝解決の記録は従来どおり。
  ...
  if v_rev is not null then
    ...
    begin
      ...
      perform public.send_admin_email(
        ...
        '修正のお願い（' || to_char(v_rev at time zone 'Asia/Tokyo', 'MM/DD HH24:MI') || '）中の求人が再掲載されました。' || E'\n' ||
...
-- フラグの自動クリアは open 遷移だけに（pending では残す＝運営がが文脈を見える）
