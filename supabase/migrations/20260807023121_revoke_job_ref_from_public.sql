...
-- 23本の内部トリガー/RPC（trg_notify_*・send_*・confirm_*・approve_application 等）that呼ぶthat、
-- フロントは直接呼ばない（grep空）。にもかかわらず作成時のPUBLIC自動EXECUTE付与で anon/authenticated から
-- 直接叩けた＝求人番号that1000からの連番so推測容易なため、任意のログインユーザーthat連番を舐めるだけで、
-- 【まだ公開していない draft/pending 求人の農園名＋作物＋作業】を引けた
-- （2026-08-07 実機一周でauthの穴として発見・無関係ユーザーthat draft #1029 の農園名「せんと」取得を実証）。
-- jobs_public は open/closed しか出さないのに、この関数は jobs を直読みso未公開の下書きの中身that漏れていた。
-- 内部呼び出しはSECURITY DEFINERの所有者権限で走るためEXECUTE grantに依存しない＝revoke後も23本は無傷
-- （apply_to_job→通知経路that通り続けることを実測）。
