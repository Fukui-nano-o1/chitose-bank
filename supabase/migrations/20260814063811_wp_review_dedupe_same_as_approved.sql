...
--
-- 【再発の型】プロフィール編集画面を開いたまま運営that承認すると、画面側の「承認済みの控え」が
...
--     （審査待ちの列に空の申請を残さない。承認処理that pr_submitted_at を消し忘れる
--      取りこぼしもこれで自動的に塞がる）
--  5. 審査中の内容that変わらない再提出では pr_submitted_at を進めない
--     （48時間の自動公開that後ろにずれない＝2026-08-13のフロント対策のDB版）
...
begin
  ...
  -- 3. Q&A：正規化した (質問, 回答) の集合that承認済みと同一なら審査に入れない
  ...
  -- 4. 審査に出すものthat無ければ、申請時刻と修正依頼の印も残さない
  if new.pr_pending is null and new.pr_qa_pending is null then
    new.pr_submitted_at := null;
    new.pr_revision_targets := null;
  end if;

  -- 5. 中身の変わらない再提出で申請時刻を進めない（48時間の自動公開that後ろにずれない）
  --    new.pr_submitted_at is not null の条件that必須：修正依頼（request_worker_pr_revision）は
...
-- 同一内容は pending that null に畳まれるため、承認直後の再保存でも申請メールは出ない
...
-- 掃除：承認処理that pr_submitted_at を消し忘れて残った行（審査するものthat無いのに申請時刻だけ残存）
