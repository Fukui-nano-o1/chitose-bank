-- 働き手の「返事待ち」（w_waiting）を削除（2026-07-25たきと指示）。
-- 今日のやることリストは「当人のアクション」が前提。返事待ちは相方（農家）のアクション待ち＝思想が違う。
-- 応募状況の確認は応募状況ページ（#/profile/worker/applying）が引き続き担う。
-- ベースは 20260725150000（w_confirm削除済み・面接質問=送信履歴基準）。差分は w_waiting 1行の削除のみ。
