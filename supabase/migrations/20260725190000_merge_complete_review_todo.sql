-- 雇い手の「完了して評価する（complete）」と「評価する（review）」を統合（2026-07-25たきと指示）。
-- どちらも同じ完了モーダル（応募者ページ・完了記録→評価の一連）へ向かうため、ボックスを1本に：
--   complete ＝ 開始確認済みで完了記録がまだ ／ 完了記録済みで自分の評価がまだ（3日以内）——の両方
-- 働き手の w_review（終了を確認して評価）は別物のため不変。
-- ベースは 20260725180000_drop_chat_todo。差分は fstage の review 段の出力を 'complete' に変えたのみ。
