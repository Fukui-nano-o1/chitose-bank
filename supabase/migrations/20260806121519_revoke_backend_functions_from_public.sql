...
-- （2026-08-06・実機一周後の点検 audit.sql ③b が検出）
--
-- 【何が問題か】送信・cron・内部の関数が、Postgresのデフォルト（作成時にPUBLICへEXECUTE付与）の
...
-- PUBLIC へ EXECUTE が自動付与される。ので `revoke ... from anon, authenticated` では
-- PUBLIC 経由の権限が残り効かない（当日1度 from anon,authenticated で当ててノーオペを実測→
...
--   ・内部呼び出し（confirm_insurance 等が perform public.send_user_email）は
--     SECURITY DEFINER 関数の中ので、関数所有者の権限で実行される＝EXECUTE権限に依らない（ok=true）
--   ・anonからの直叩きは権限エラーで拒否されることを実測
--   ・フロントからの直接呼び出しは無い（send_interview_questions のみthaフロント呼び出し＝対象外ので残す）
