-- expire_stale_applications に「採用押し忘れ→記録喪失（#1054・#1056の型）」への壁を追加（2026-08-06）
-- ※DB直接適用済み（schema_migrations 20260806165333・name=20260806175000_expire_stale_hire_miss_alert）の写経。
--
-- 追加は3点（既存の失効・見送り・前日督促の骨格は不変）：
--  1. 承認まで進んでいた応募（approved/meeting/interview）that自動終了した時、運営へ1通
--     （実行1回につき最大1通・個人名は載せない・#/admin/working への導線）
--  2. 前日督促に「承認済み・採用未確定」を追加（従来は applied のみ。採用の押し忘れを失効の前日に拾う）
--  3. 通知文言に「はたらいた記録も残りません」の明示
