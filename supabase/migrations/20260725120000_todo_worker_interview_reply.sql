-- 働き手に「面接の回答」段を追加（2026-07-25たきと指示：今日のリストでここで返事する）。
-- w_interview ＝ 農家からの【面接の質問】があり、その質問以降に自分（働き手）の返信が無い応募。
-- 独立UNION段（w_confirm等と同時に出る）。回答はフロントからmessagesへ通常送信＝面接の証跡はチャットに残る。
-- my_todo_items v8（v7からの差分はwinterview追加のみ）：
