-- 「完了して評価する」が評価後も消えない不具合の修正（2026-08-06・実機一周で発見）
--
-- 【症状】農家が完了記録＋評価を送った後も、今日ページのやること「完了して評価する」が
-- 3日間残り続ける（件数バッジも減らない）。
--
...
-- 副問い合わせの中では、最も内側のテーブル（reviews）が優先して解決されるため、
...
--
-- 【実測】修正前：評価送信後も complete が1件残る／`a.id` で書いた同じ述語は0件を返す。
