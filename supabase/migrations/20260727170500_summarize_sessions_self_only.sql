-- summarize_sessions の対象を反転（2026-07-27たきと指示・20260727170000と対）
-- 従来：管理者を除く全員＝利用者の軌跡を運営に送っていた（憲法違反）
-- 以後：運営者本人（app_admins）のセッションだけを要約する。分割送信の仕様は20260727160000のまま
  ...
  for r in
    select auth_id from public.page_events
     where not summarized
       and exists (select 1 from public.app_admins a where a.auth_id = page_events.auth_id)
     group by auth_id
